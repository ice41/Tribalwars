...
{else}
	{if empty($error)}
		...
		<td align="center" width="120">{if $mail.from_id==-1}Odpisz{else}<a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=new&amp;reply={$mail.id}">Odpisz</a>{/if}</td>
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=new&amp;reply={$mail.id}&amp;forward=true">Napisz</a></td>
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;action=arch&amp;id={$mail.id}&amp;h={$hkey}">Archiwum</a></td>
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;action=del&amp;id={$mail.id}&amp;mode=in&amp;h={$hkey}">Usuñ</a></td>
		</tr>
		</table>
		
		<table class="vis" width="100%">
		<tr><th width="140">Temat</th><th width="300">{$mail.subject}</th></tr>
		<tr><td>Odbiorca</td><td>{if $mail.from_id==-1}{$mail.from_username}{else}<a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.from_id}">{$mail.from_username}</a>{/if}</td></tr>
		...
		<td align="center">{if $mail.from_id==-1}Zablokuj nadawcê{else}<a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=block&amp;action=block_id&amp;id={$mail.from_id}&amp;h={$hkey}">Zablokuj nadawcê</a>{/if}</td>
		</tr>
		</table><br />
	{/if}
{/if}