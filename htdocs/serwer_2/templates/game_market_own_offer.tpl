...
{if count($offers)>0}
	...
	<tr><th>Oferta</th><th>za</th><th>Ile razy</th><th>Data ustawienia</th></tr>
		{foreach from=$offers item=arr key=id}
			<tr><td><input name="id_{$id}" type="checkbox" />{if $arr.sell_ress=='wood'}<img src="graphic/holz.png" title="Drewno" alt="" />{/if}{if $arr.sell_ress=='stone'}<img src="graphic/lehm.png" title="Glina" alt="" />{/if}{if $arr.sell_ress=='iron'}<img src="graphic/eisen.png" title="¯elazo" alt="" />{/if}{$arr.sell} </td>
			<td>{if $arr.buy_ress=='wood'}<img src="graphic/holz.png" title="Drewno" alt="" />{/if}{if $arr.buy_ress=='stone'}<img src="graphic/lehm.png" title="Glina" alt="" />{/if}{if $arr.buy_ress=='iron'}<img src="graphic/eisen.png" title="¯elazo" alt="" />{/if}{$arr.buy} </td>
			<td>{$arr.multi}</td>	
			<td>{$arr.time}</td>
			</tr>
		{/foreach}
