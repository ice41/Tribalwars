function ajaxFunction(){
  ...
  try{
    ...
    // Wy³apuje b³¹d jeœli JavaScript nie posiada obiektu XMLHttpRequest
    try {
      ...
      try {
        ...
        // Wyœwietlenie b³êdu o braku obs³ugi obiektu XMLHttpRequest
...
KAMap.prototype.tooltipDetails = function(village_id, village_name, points, owner, ally, moral, grocusto, runtimes, ressis, settlers, mules, units, isProtectedFromAttacks, isProtectedFromEnnoblings, BonusText, BonusImage) {
    showUnits = (units || runtimes);
    var i = 0;
    ...
    if (showUnits) {
        ...
        for (i = 0; i < 12 ;i++ ) {
            ...
            if (i < 12 && !units && typeof(speed[unit]) == 'undefined') {
                continue;
            }
    ...
    code = '';
    code += '<table class="vis">';
	
	if (BonusImage) {
		 code += '<tr><td rowspan="10"><img src="' + BonusImage + '" alt=""/></td></tr>';
		 }
	
    code += '<tr><th colspan="2">' + village_name;

    if (isProtectedFromAttacks) {
        code += '<img src="/ds_graphic/ochpocz.png">';
    }

    code += '</th></tr>';
	
	if (BonusText != "") {
		code += '<tr><td colspan="2" style="font-weight:bold;">' + BonusText + '</td></tr>';
		}
    ...
    if (grocusto) {
        code += '<tr><td>' + lang['GROUPS'] + ':</td><td>' + grocusto + '</td></tr>';
    }
    if (ressis) {
        ress = ressis.split(':');
        code += '<tr><td colspan="2">';
        code += '<table class="noborder"><tr>';
        code += '<td><span class="icon header wood"></span></td><td>' + ress[0] + '</td>';
        code += '<td><span class="icon header stone"></span></td><td>' + ress[1] + '</td>';
        code += '<td><span class="icon header iron"></span></span></td><td>' + ress[2] + '</td>';
        code += '<td><span class="icon header res"></span></td><td>' + ress[3] + '</td>';
        code += '</tr></table>';
        code += '</td></tr>';
    }
    ...
    if (settlers) {
        ...
        if (settlers) {
            code += '<td><span class="icon header population"></span></td><td>' + settlers + '</td>';
        }
    ...
    if (showUnits) {
        ...
        for (i = 0; i < 12 ;i++ ) {
            ...
            unit = this.options.units[i];
            code += '<td style="text-align:center;"><img src="/ds_graphic/unit/' + unit + '.png" /></td>';
        }
        code += '</tr>';
        ...
        if (runtimes) {
            ...
            for (i = 0; i < 12 ;i++ ) {
                ...
                if (typeof(speed[unit]) == 'undefined') {
                    ...
                    style = '';
                    if (unit == 'unit_snob' && dist > this.options.snob_max_distance) {
                        style = ' style="color:red;"';
                    }
                    code += '<td class="text_info"' + style + '>' + formatTime2( Math.round(dist * speed[unit]) ) + '</td>';
                }
            }
            code += '</tr>';
        }
    ...
    if (showMules) {
        ...
        code += '<table class="noborder">';
        code += '<tr>';
        code += '<td style="text-align:center;"><img src="/ds_graphic/bonus/storage_mini.png" /></td>';
        code += '</tr>';
        ...
        if (runtimes) {
            ...
            if (typeof(speed['donkey']) == 'undefined') {
                ...
                code += '<td class="text_info">' + formatTime2( Math.round(dist * speed['donkey']) ) + '</td>';
            }
...
config. BgColor         = '#EDE7C7' // Background colour (HTML colour value, in quotes)
