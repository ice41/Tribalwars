...
{if $show_build}
    {php}
	$this->_tpl_vars['pl_trans'] = array("send" => "Wyœlij surowce","own_offer" => "W³asne oferty","other_offer" => "Obce oferty");
	{/php}
			...
			{if $f_mode==$mode}
				<tr>
					<td class="selected" width="120">
						<a href="game.php?village={$village.id}&amp;screen=market&amp;mode={$f_mode}">{$pl_trans.$f_mode}</a>
					</td>
				</tr>
			{else}
                <tr>
