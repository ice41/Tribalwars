...
if ($_GET['mode'] == 'poj_monety') {
	...
	if ($_GET['action'] == 'cancel') {
		...
		if ($counter > 0) {
			...
			$cts = $info_trenowaniu['time_finished'] - $info_trenowaniu['time_start'];
	...
	$szlachta_trening = sql("SELECT `num_unit` - `num_finished` FROM `recruit` WHERE `userid` = '".$user['id']."' AND `building` = '".$_GET['screen']."'",'array');
	...
	if ($_GET['action'] == 'train' and isset($_POST)) {
		$_error = 0;
...
if ($_GET['mode'] == 'mass_monety') {
	...
	if ($_GET['action'] == 'wybijaj_wiele_monet' && isset($_POST)) {
		$monety_dod = 0;
