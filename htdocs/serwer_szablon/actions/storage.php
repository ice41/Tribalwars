...
for ($i = $village[$_GET['screen']]; $i <= $i_conq; $i++) {
	...
	$storage_arr[$i]['produkcja'] = number_format(floor($arr_maxstorage[$i]));
...
if ($village['r_wood'] >= $full_storage) {
	...
	$spich['time_wood'] = floor(($spich['do'] / ($arr_production[$village['wood']] * $config['speed'])) * 3600);
...
if ($village['r_stone'] >= $full_storage) {
	...
	$spich['time_stone'] = floor(($spich['do'] / ($arr_production[$village['stone']] * $config['speed'])) * 3600);
...
if ($village['r_iron'] >= $full_storage) {
	...
	$spich['time_iron'] = floor(($spich['do'] / ($arr_production[$village['iron']] * $config['speed'])) * 3600);
