...
if ($_GET['try'] === 'back' && isset($_GET['unit_id'])) {
	...
	if ($_GET['action'] === 'back' && isset($_GET['unit_id']) && count($_POST) > 0) {
		if ($_GET['h'] == $session['hkey']) {
			...
			// Verifique se os dados corretos são fornecidos:
				$errors = 0;
				$sum_units = 0;
				foreach ($cl_units->get_array("dbname") as $dbname) {
				...
				if ($errors > 0) {
					$error = 'O número de unidades foi dado.';
					} else {
					if ($sum_units > 0) {
						...
						$error = 'Você deve enviar pelo menos uma unidade.';
						}
					}
				} else {
				$error = 'Será tratado.';
	...
	// Escolha unidades que vêm da aldeia atual:
	$this_units = $cl_units->read_units($village['id'],$village['id']);

	// Crie uma placa com unidades estacionadas na aldeia atual:
	...
	//Utwurz tablic� z jednostkami z aktualnej wioski, kt�re stacjonuj� w innych wioskach:
