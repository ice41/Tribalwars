...
if ($_GET['action'] == 'edit' and isset($_POST)) {
	...
	$arr = implode(';',$arr);
	$p = fopen('admin/bns.xth','w');
	fputs($p,$arr);
	fclose($p);
	

	$p = fopen('admin/pts.xth','w');
	fputs($p,$vill_points);
	fclose($p);
	...
	header('location: game.php?village='.$_GET['village'].'&screen=admin&mode=builds');
	EXIT;
    }
$arr = explode(';',file_get_contents('admin/bns.xth'));
...
$text = 'Esta página permite que edite facilmente os edifícios iniciais para <b>aldeias de jogadores</b> e <b>aldeias bárbaras</b>!';
