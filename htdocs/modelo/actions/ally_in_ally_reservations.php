...
if ($_GET['rlog']) {
	...
	if ($_GET['action'] == 'add_partner' && $is_user_admin && count($_POST) > 0) {
		if ($_GET['h'] == $session['hkey']) {
			...
			if ($ally_counts < 1) {
				$error = 'O nome da tribo fornecido não existe';
				} else {
				...
				if ($ally_id == $user['ally']) {
					...
					if ($counts_in_dzielenie_rezerwacji < 1) {
						...
						$error = 'Já está compartilhando reservas com a tribo '.entparse($_POST['partner_ally']);
						}
					}
				}	
			} else {
		...
		if ($_GET['h'] == $session['hkey']) {
			if ($_POST['typ_akcji'] === 'add_new') {
				if (count($_POST['x']) < 10) {
					...
					$error = 'Pode adicionar até 10 reservas por vez';
					}
				}
				...
				if ($res_info['od_plemienia'] == $user['ally']) {
					if ($is_user_admin) {
						...
						if ($res_info['od_gracza'] == $user['id']) {
							...
							$error = 'Não pode excluir esta reserva';
							}
						}
					} else {
					$error = 'Não existe essa reserva';
					}
				} else {
				$error = 'Não existe essa reserva';
				}
	...
	//Utwurz tablic� z dzieleniem rezerwacji od plemienia:
	...
	//Utwurz tablic� z dzieleniem rezerwacji do plemienia:
	...
	if (isset($_POST['search_reservations_is'])) {
		if ($_POST['reservation_search_coords'] === 'true') {
			...
			if ($_POST['group_id'] === 'creator_name') {
				...
				$_mysql_where_tag = "WHERE (`od_gracza` = '".$pid."') AND ($sql_query_filters$sql_query_filters)";
