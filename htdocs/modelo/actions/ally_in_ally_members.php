	...
	if ($_GET['action'] === 'mod' && count($_POST) > 0) {
		if ($_GET['h'] == $session['hkey']) {
			...
			if ($count > 0) {
				if ($_POST['ally_action'] === 'kick') {
					if ($_POST['sel_player_id'] != $user['id']) {
						...
						if ($sel_found && !$user['ally_found']) {
							$error = "Nie mo�na wyprosi� gracza o wy�szym stopniu";
							} else {
							if ($sel_admin && $user['ally_lead'] && !$user['ally_found']) {
								$error = "Nie mo�na wyprosi� gracza o stopniu Admina lub Za�o�yciela!";
								} else {
								...
								$title = '<a href="game.php?village=;&screen=info_player&id='.$_POST['sel_player_id'].'">' . entparse($pname). '</a>' . ' foi expulso da tribo por ' .
				...
				$error = 'Um jogador não foi selecionado';
				}
			} else {
				...
				if ($sel_found && !$user['ally_found']) {
					...
					if ($sel_admin && $user['ally_lead'] && !$user['ally_found']) {
					    ...
					    if ($found_counts <= 1 && $sel_found && !isset($_POST['player_id'][$uid]['found'])) {
							$error = 'Musi zosta� co najmniej jeden za�o�yciel.';
							} else {
