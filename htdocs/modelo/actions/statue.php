...
$f_modes = array('main' => 'Paladino','inventory' => 'Arsenal');
...
if ($_GET['mode'] == 'inventory') {
	...
	if ($_GET['action'] == 'cancelar' and isset($_GET['id']) && $_GET['h'] == $session['hkey']) {
		$_GET['id'] = (int)$_GET['id'];
	...
	if ($_GET['action'] == 'change_pala_name' && count($_POST) > 0 && $user['paladins'] == 1 && $_GET['h'] == $session['hkey']) {
		...
		if ($_POST['nazwa'] === 'SHORT') {
			$error = 'O nome do seu cavaleiro não pode estar vazio!';
			}
		elseif ($_POST['nazwa'] === 'LONG') {
			$error = 'O nome do seu cavaleiro não pode exceder 35 sinais!';
			}
		elseif ($_POST['nazwa'] === 'SPACES') {
			$error = 'O nome do seu cavaleiro não pode consistir nos espaços';
			} else {
