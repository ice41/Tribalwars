...
class DB_MySQL {
	...
	function connect($host=null, $username=null, $password=null, $db=null) {
		...
		if (!$this->connection) {
			//echo mysql_error();
			printf('Você não pode se conectar com o MySQL.(Está ativado?) <br /> Blad: %s',
				htmlentities(mysql_error())
			);
			exit;
		}
		if (!@mysql_select_db($db, $this->connection)) {
			printf('Não é possível selecionar o banco de dados "%s" (ele existe?) <br /> Mensagem de erro: %s', htmlentities($db), htmlentities(mysql_error()));
			exit;
		}
		return true;
	}
	...
	function unb_query($sql, $showError=true, $errorIsFatal=true) {
        ...
        function handle_query_result($result, $sql, $showError=true, $errorIsFatal=true) {
            if (!$result) {
                if ($showError) {
                    printf('Failing SQL: %s<br />Error: %s', htmlentities($sql), htmlentities(mysql_error($this->connection)));
                    if ($errorIsFatal) {
			exit;
                    }
		}
	...
	function query_safe($sql) {
		if (!is_string($sql)) {
			...
			printf("DB::query_safe: O primeiro argumento esperado é uma string\n");
			return null;
		}
		$args = func_get_args();
		// parameter 0 is $sql
