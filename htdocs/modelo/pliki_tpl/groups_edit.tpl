...
<div id="group_config" class="group_config" style="display: none;">
	...
	<table class="vis" width="100%"/>
		...
		{foreach from=$user_grocusto item=group_name}
			<tr height="23">
				...
				<td {if $user.aktu_group === $group_name}class="selected"{/if}><a href="game.php?village{$village.id}&screen={$screen}&mode={$mode}&amp;action=del_group&group={php} echo base64_encode($this->_tpl_vars['group_name']);{/php}&amp;h={$hkey}"/><img src="/ds_graphic/delete.png" alt="Excluir"/></a></td>
			</tr>
		{/foreach}
		<tr height="20">
			<td {if $user.aktu_group === 'all'}class="selected"{/if}><a href="game.php?village{$village.id}&screen={$screen}&mode={$mode}&amp;action=change_group&group={php} echo base64_encode('all');{/php}&amp;h={$hkey}"/>Tudo</td>
