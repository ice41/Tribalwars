{if $pok_tem}
	...
	<h2>Utwórz now¹ ankietê</h2>

	<form action="{$base_link}&sm=new_pool&aktu_fid={$aktu_fid}&action=write&h={$hkey}" method="post" name="new_pool">
		...
		<table class="vis" id="formTable">
			<tbody>
				<tr>
					<td>Assunto:</td>
					<td><input name="subject" size="64" value="{$subject}" tabindex="1" type="text"></td>
				</tr>
				<tr>
					<td valign="top">
						Odpowiedzi:<br>
						<center>
							<a href="#" onclick="Forum.addPollOption(); return false;">Adicionar</a>
						</center>
					</td>
					<td>
						<div id="poll_options">
							{foreach from=$options item=value}
								<input name="poll_options[]" size="64" value="{$value}" maxlength="50" class="answer" tabindex="3" type="text"><br>
							{/foreach}
						</div>
					</td>
				</tr>
				<tr>
					<td></td>
					<td>
						<label><input name="show_result" tabindex="999" type="checkbox" {if $show_result}checked="checked"{/if}>Poka¿ wynik tak¿e tym graczom, którzy jeszcze nie g³osowali.</label>
					</td>
				</tr>
					...
					<td>													
						<div id="bb_bar" style="overflow: visible; text-align: left;">
							...
							<div id="bb_color_picker" style="display: none; clear: both;">
								...
								<input id="bb_color_picker_tx" type="text"><input class="btn" value="OK" id="bb_color_picker_ok" onclick="BBCodes.colorPickerToggle(true);return false;" type="button">
							</div>
						</div>
					</td>
				...
				<tr>
					...
					<td>
						<label><input name="important" tabindex="3" type="checkbox" {if $important}checked="checked"{/if}>Wa¿ne</label>
					</td>
				</tr>
			</tbody>
		</table>
