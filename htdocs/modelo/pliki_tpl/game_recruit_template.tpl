					...
					<table class="vis">
						...
						{foreach from=$recruit_units key=key item=value}
							<tr {if $recruit_units.$key.lit}class="lit"{/if}>
								...
								<td><a href="game.php?village={$village.id}&amp;screen={$dbname}&amp;action=cancel&amp;id={$key}&amp;h={$hkey}">parar</a></td>
							</tr>
						{/foreach}
					</table>
				</div>
		...
		<table class="vis">
			...
			{foreach from=$units key=unit_dbname item=name}
				<tr>
					...
					<td><img src="/ds_graphic/holz.png" title="Madeira" alt="" /> {$cl_units->get_woodprice($unit_dbname)}</td>
					<td><img src="/ds_graphic/lehm.png" title="Argila" alt="" /> {$cl_units->get_stoneprice($unit_dbname)}</td>
					<td><img src="/ds_graphic/eisen.png" title="Ferro" alt="" /> {$cl_units->get_ironprice($unit_dbname)}</td>
					<td><img src="/ds_graphic/face.png" title="População" alt="" /> {$cl_units->get_bhprice($unit_dbname)}</td>
