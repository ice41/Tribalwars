...
<tr><th colspan="3">Convites</th></tr>
	{foreach from=$invites item=arr key=id}
		<tr>
			...
			<td>{$arr.time}</td>
			<td><a href="game.php?village={$village.id}&amp;screen=ally&amp;mode=invite&amp;action=cancel_invitation&amp;id={$id}&amp;h={$hkey}">Wycofa�</a></td>
		</tr>
	{/foreach}
</table>
<br />
<form action="game.php?village={$village.id}&amp;screen=ally&amp;mode=invite&amp;action=invite&amp;h={$hkey}" method="post">
	<table class="vis" width="400">
