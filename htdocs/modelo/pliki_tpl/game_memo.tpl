...
<input id="rename_tab_url" value="game.php?village={$village.id}&amp;ajaxaction=rename_tab&amp;screen=memo" type="hidden">
...
<div id="tab-bar">
			<div id="tab_{$user.id}" class="memo-tab  memo-tab-selected"><span class="memo-tab-label"><strong>Nova aba</strong></span></div>
	</div>

<div id="memo_{$user.id}" class="memo_container" style="">
<div class="memo_script" style="clear: both;">
	...
	<form action="game.php?village={$village.id}&amp;screen=memo&amp;action=edit&amp;h={$hkey}" method="post">
		<input name="tab_id" value="{$user.id}" type="hidden">
...
{$memo_viev}
			...
			<tr class="edit_row" style="display:none">
				<td colspan="2">
					<textarea id="message_{$user.id}" name="memo" cols="80" rows="25">{$memo_bb}</textarea>
				</td>
			</tr>
			...
			<tr class="submit_row" style="display:none">
				<td>
					<input id="submit_memo_{$user.id}" class="btn" value="Salvar" type="submit">
...
<script type="text/javascript">
//<![CDATA[
	var tab_id = '#message_{$user.id}';
	var premium = true;
	var char_limit = 5000;

	
	$('document').ready(function() {ldelim}
		Memo.init([{ldelim}"id":"{$user.id}","title":"{$user.memo_name}","memo":"{$memo_bb}","memo_bb":"{$memo_viev}"}],false, false);
