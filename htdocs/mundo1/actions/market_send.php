<?php
//Je�li w�amano si� bezpo�rednio do tego pliku, to oddaj error:
if (!is_object($bonus)) {
	die("Erro de acesso!");
	}
...
if ($_GET['try'] === 'confirm_send' && count($_POST) > 0 && isset($_POST['x']) && isset($_POST['y'])) {
	...
	if (is_array($effect)) {
		...
		// Calcule o tempo do caminho para comprar:
		...
		if ($_GET['h'] == $session['hkey']) {
			...
			if ($czy_wlasny > 0) {
				...
				if ($trn_time_as_start > ($config['cancel_dealers'] * 60)) {
					$error = 'Você não pode derramar um determinado transporte';
					} else {
					$start_time = date("U");
