...
//<![CDATA[
	...
	TWMap.urls.ctx = {ldelim}};
	TWMap.urls.ctx.mp_overview = 'game.php?village=__village__&screen=overview';
	TWMap.urls.ctx.mp_info = 'game.php?village={$village.id}&id=__village__&screen=info_village';
	TWMap.urls.ctx.mp_fav = 'game.php?village={$village.id}&id=__village__&ajaxaction=add_target&h={$hkey}&json=1&screen=info_village';
	TWMap.urls.ctx.mp_unfav = 'game.php?village={$village.id}&id=__village__&ajaxaction=del_target&h={$hkey}&json=1&screen=info_village';
	TWMap.urls.ctx.mp_lock = 'game.php?village={$village.id}&id=__village__&ajax=rezerwuj&h={$hkey}&json=1&screen=info_village';
	TWMap.urls.ctx.mp_res = 'game.php?village={$village.id}&mode=send&target=__village__&screen=market';
	TWMap.urls.ctx.mp_att = 'game.php?village={$village.id}&target=__village__&screen=place';
	TWMap.urls.ctx.mp_recruit = 'game.php?village=__village__&screen=train';
	TWMap.urls.ctx.mp_Perfile = 'game.php?village={$village.id}&id=__owner__&screen=info_player';
	TWMap.urls.ctx.mp_msg = 'game.php?village={$village.id}&mode=new&player=__owner__&screen=mail';
	TWMap.urls.ctx.mp_unlock = TWMap.urls.ctx.mp_lock;
	TWMap.urls.ctx.mp_invite = 'game.php?village={$village.id}&mode=ref&source=map&screen=settings';
	TWMap.urls.ctx.mp_invite_hide = 'game.php?village={$village.id}&ajaxaction=map_hide_invitation&h={$hkey}&json=1&screen=settings';
	...
	TWMap.urls.villageInfo = 'game.php?village={$village.id}&id=__village__&screen=info_village';
	TWMap.urls.villagePopup = 'game.php?village=__village__&ajax=map_info&source={$village.id}&screen=overview';
	TWMap.urls.sizeSave = 'game.php?village={$village.id}&ajaxaction=set_map_size&h={$hkey}&screen=settings';
	TWMap.urls.changeShowBelief = 'game.php?village={$village.id}&ajaxaction=change_topo_show_belief&h={$hkey}&screen=settings';
	TWMap.urls.changeShowPolitical = 'game.php?village={$village.id}&ajaxaction=change_topo_show_political&h={$hkey}&screen=settings';
	TWMap.urls.changeUseContext = 'game.php?village={$village.id}&ajaxaction=change_use_contextmenu&h={$hkey}&screen=settings';
	TWMap.urls.savePopup = 'game.php?village={$village.id}&ajax=save_map_popup&screen=map';
	TWMap.urls.centerCoords = 'game.php?village={$village.id}&mode=centerlist&screen=map'
	TWMap.urls.centerSave = 'game.php?village={$village.id}&mode=centerlist&ajaxaction=save_center&h=b036&screen=map';
	...
	TWMap.secrets = {ldelim}
	{foreach from=$odk item=od}{$od.wioska}: [{$od.i},{$od.c1},{$od.c2}]{if count($odk) != $od.counter},{/if}{/foreach} };
...
width:290px; z-index:9999; display:none">
 ...
 <td valign="top">
  ...
  <form method="post" action="game.php?village={$village.id}&amp;type=for&amp;action=activate_group&amp;h=b036&amp;screen=map">
  <table class="vis" id="for_grocusto">
   <tbody>
	 ...
	 <td class="small" width="15">
		<a href="#" onclick="ColorGrocusto.edit_color_toggle($(this), 'game.php?village={$village.id}&ajaxaction=load_edit_color&h=b036&screen=map', 9572, 254, 0, 254, null, false);return false;" style="display: block;width:29px;height:18px;background-image:url('/graphic/colorpicker.png')">
	 ...
	 <td class="small" width="15">
		<a href="#" onclick="ColorGrocusto.edit_color_toggle($(this), 'game.php?village={$village.id}&ajaxaction=load_edit_color&h=b036&screen=map', 20388, 0, 254, 0, null, false);return false;" style="display: block;width:29px;height:18px;background-image:url('/graphic/colorpicker.png')">
  ...
  <a href="#" onclick="javascript:ColorGrocusto.add_for_village();return false">&raquo; Um novo grupo</a>
		...
		//<![CDATA[
			{foreach from=$kontrakty.partner item=kontrakt}
					TWMap.allyRelations[{$kontrakt.do_plemienia}] = 'partner';
		    {/foreach}
			{foreach from=$kontrakty.nap item=kontrakt}
					TWMap.allyRelations[{$kontrakt.do_plemienia}] = 'nap';
		    {/foreach}	
			{foreach from=$kontrakty.enemy item=kontrakt}
					TWMap.allyRelations[{$kontrakt.do_plemienia}] = 'enemy';
		    {/foreach}
			{foreach from=$rezerwacje item=r}
					TWMap.reservations[{$r.do_wioski}] = '{$r.od}';
		    {/foreach}			
...
<table class="vis" width="100%">
	...
	<tr>
		...
		<td colspan="2">
			...
			<input type="hidden" value="game.php?village={$village.id}&amp;ajaxaction=set_map_size&amp;h={$hkey}&amp;screen=settings" id="change_map_size_link" />
		</td>
	</tr>
	</table>
...
$(document).ready(function() {ldelim}
	...
	TWMap.autoSize = Math.ceil(TWMap.autoPixelSize / TWMap.tileSize[0]);
			...
			TWMap.size = [{$user.map_size},{$user.map_size}];
	...
	TWMap.init();
	TWMap.focus({$village.x}, {$village.y}, false);
	...
	if(mobile) {ldelim}
		...
		window.addEventListener(flipEvent, function() {ldelim}
			var autoSelected = (parseInt($('#map_chooser_select').val()) == 0);
			if(autoSelected) {ldelim}
				if (resizeTimer === null) {ldelim}
					resizeTimer = setTimeout(function() {ldelim}
