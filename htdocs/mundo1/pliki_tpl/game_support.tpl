...
{/php}
<h3>{php} echo urldecode($subject_topic); {/php}</h3>
<tr>
	<th>{php}
$id_s=$this->_tpl_vars['village']['id'];
	echo '<a href="game.php?village='.$id_s.'&screen=info_player&id='.$uid_topic.' "> '.$username_topic.'</a>'; echo "<font size='1'> ".$date_topic; {/php}</font></th>
...
$sql3 = "select * from `support_post` where  id_ticket ='$id'";
	   ...
	   foreach ($id_q as $key => $value) {
		...
		{/php}
<tr>
	<th>{php} if ($id_q[$key] == '-1') { echo '<font color="green">'.$nume[$key].'</font>'; } else { echo '<a href="game.php?village='.$id_s.'&screen=info_player&id='.$id_q[$key].' "> '.$nume[$key].'</a>'; } echo "<font size='1'> ".$date[$key]; {/php}</font></th>
	  ...
	  if ($select_inchis != '0')
{
{/php}<table> <Br /><h3>Tópico fechado!</h3> </table>
{php}
}
else
{
	  ...
	  <br>
<form method=post action="game.php?village={$village.id}&screen=support&view=ticket&do=post&id={php} echo $id; {/php}">
...
<form method=post action="game.php?village={$village.id}&screen=support&do=new_ticket&open">
...
$sql3 = "select * from `support` where  uid = '$id_user'";
	   ...
	   foreach ($id_q as $key => $value) {
			 ...
			 {/php}
	<tr>
			<td><a href="game.php?village={$village.id}&screen=support&view=ticket&id={php} print $id_q[$key]; {/php}">
...
if ($new[$key] == '0')
			...
			<img src="../graphic/forum/{php} echo $read; {/php}">
			{php}
			echo stripslashes(urldecode($subject[$key])); {/php}</td>
			<td>{php} echo $date[$key]; {/php}</td>
			</tr>
			{php}
			 }
      } }
