...
<p><p><h2>Entrar com contacto com o suporte para saber como adquirir o codigo:</h2>
	...
	<tbody><tr>
		...
		<tr><td>50 <td>Moedas<td>{$kod.1.zl} €<td>50 PP{if $event_p}<font color="green"><b>+ 50</b></font>{/if}<td><b><center><font color="{if $kodb.1 <= 10}red{else}green{/if}">{$kodb.1}</font></td>
		<tr><td>150 <td>Moedas<td>{$kod.2.zl} €<td>150 PP{if $event_p}<font color="green"><b>+ 150</b></font>{/if}<td><b><center><font color="{if $kodb.2 <= 10}red{else}green{/if}">{$kodb.2}</font></td>
		<tr><td>300 <td>Moedas<td>{$kod.3.zl} €<td>300 PP{if $event_p}<font color="green"><b>+ 300</b></font>{/if}<td><b><center><font color="{if $kodb.3 <= 10}red{else}green{/if}">{$kodb.3}</font></td>
		<!--<tr><td>1000 <td>Moedas<td>{$kod.4.zl} €<td>1000 PP{if $event_p}<font color="green"><b>+ 1000</b></font>{/if}<td><b><center><font color="{if $kodb.3 <= 10}red{else}green{/if}">{$kodb.3}</font></td>-->
	</tr>
