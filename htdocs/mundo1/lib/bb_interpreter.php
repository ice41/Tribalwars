...
class bb_interpreter {
	...
	function bb_interpreter($cl_builds,$cl_units,$pl) {
		if (is_object($cl_builds)) {
			...
			$this->fatal_error("A variável cl_builds deve ser um objeto! ",__FILE__,__LINE__);
			}
		if (is_object($cl_units)) {
			$this->objects['cl_units'] = $cl_units;
			} else {
			$this->fatal_error("A variável cl_units deve ser um objeto! ",__FILE__,__LINE__);
			}
		if (is_object($pl)) {
			$this->objects['pl'] = $pl;
			} else {
			$this->fatal_error("A variável PL deve ser um objeto! ",__FILE__,__LINE__);
			}
		}
	
	function fatal_error($message,$file,$line) {
		$message = "<br />\n<b>Desastroso b��d</b>:  $message w <b>$file</b> na lini <b>$line</b><br />";
		die($message);
		}
	...
	function compile_bb_code($str,$aktu_user) {
		...
		//Je�li s� jakie� bb-tagi, to przejd� do prasowania tego stringu 
		if ($is_bb_tags && count($_BB_TAGS[1]) < 500) {
			foreach ($_BB_TAGS[1] as $_ID_NUMBER => $_TAG) {
				...
				if ($_TAG === '/url' || $_TAG === '/URL') {
					if ($this->_OPENED_TAGS_COUNT['url'] > 0) {
						...
						$str = preg_replace('/\['.validate_reg($_TAG).'\]/','" rel="nofollow" title="$1"> link externo </a>',$str,1);
						} else {
				...
				if (preg_match('~(?:quote|QUOTE)\=(.+)~s',$_TAG,$_EFFECT)) {
					...
					$BOX_VALUE = '<table class="quote"><tbody><tr><td></td><td class="quote_author">'.$_EFFECT[1].' Escreva:';
				...
				if ($_TAG === 'table' || $_TAG === 'TABLE') {
					...
					$str = preg_replace('/\['.$_TAG.'\]/','<table class="vis bbcodetable" align="center">',$str,1);
					}
				if ($_TAG === '/table' || $_TAG === '/TABLE') {
				...
				/*Wywal wszystkie specjalne tagi znajduj�ce si� w otwartym specjalnym tagu*/
			...
			//Przejd� do kolejnego procesu kompilacji bb-codes:
			...
			//Przedostatni etap kompilacji bb-codes : kompilacja specjalnych tag�w sql:
			
			//Kompilacja link�w do gracza:
			...
			if (count($villages_links_array) > 0) {
				foreach ($villages_links_array as $villagecoords) {
					...
					if (empty($villageid)) {
						...
						$str = preg_replace('/'.validate_reg('[coord]'.$villagecoords.'[/coord]').'/','<a href="game.php?village=[akuvillage]&screen=info_village&id='.$villageid.'"/>'.$coords[0].'|'.$coords[1].'</a>',$str,1);
						}
					}
				}
			
			//Kompilacja link�w do plemienia:
			...
			if (count($reports_id_array) > 0) {
				...
				foreach ($reports_id_array as $based_report_id) {
					...
					if ($report_counter > 10) {
						$str = preg_replace('/'.validate_reg('[report_display]'.$based_report_id.'[/report_display]').'/','<b>Atenção:</b> Maximamente, pode pressionar 10 relatórios!<br>',$str,1);
						} else {
						...
						if ($report_count > 0) {
							...
							$str = preg_replace('/'.validate_reg('[report_display]'.$based_report_id.'[/report_display]').'/','Nie mo�na odnale�� raportu!<br>',$str,1);
							}
						}
					}
				}
				
			//Ostatni etap kompilacji bb-codes : zamykanie tag�w, kt�re nie zosta�y zamkni�te:
			$_TAGS_BB_HTML = array (
				'b' => '</b>' , 'i' => '</i>' , 'u' => '</u>' , 's' => '</s>' , 'color' => '</span>' ,
				'size' => '</span>' , 'img' => '" alt="" title=""/>' ,
				'url' => '" rel="nofollow" title="$1"> link externo </a>' , 
		...
		if (count($_BB_TAGS[1]) >= 1000) {
			$str = 'Número de colchetes "<b>[</b>" i "<b>]</b>" não pode exceder 1000';
			}
	...
	function get_report_contents($id,$aktuuser) {
		...
		$report_content .= '<th width="140">título</th><th width="400"><img src="'.$raport['title_image'].'"/>&nbsp;';
		...
		if ($raport['wins'] == 'att') {
			$report_content .= '<h3>O agressor ganha</h3>';
			} else {
			$report_content .= '<h3>O zagueiro ganha</h3>';
			}
		...
		$report_content .= '</tr><tr class="center"><td>Ilo��:</td>';
		...
		if ($raport['a_units'][$pala_id] > 0) {
			$report_content .= '<tr class="center"><td>Paladino:</td><td colspan="'.count($this->objects['cl_units']->get_array("dbname")).'">';
			if ($raport['a_units'][$pala_id] == $raport['b_units'][$pala_id]) {
				if ($aktuuser == $raport['from_user']) {
					$report_content .= 'Seu paladino vai morrer.';
					} else {
					$report_content .= 'O Paladino foi morto.';
					}
				} else {
				$report_content .= 'Paladino nomeado '.$raport['att_pala_name'].' ,';
				if (!empty($raport['att_pala_item'])) {
					$report_content .= 'equipado com '.$pala_bonus[$raport['att_pala_item']][2];
					} else {
					$report_content .= 'sem o item.';
					}
				}
		...
		$report_content .= '<table width="100%" style="border: 1px solid #DED3B9"><tr><th width="100">protetor:</th><th>';
		...
		if ($raport['to_user'] == $aktuuser || $raport['wins'] == 'att' || ($raport['a_units'][$spy_id] - $raport['b_units'][$spy_id]) > 0) {
			...
			$report_content .= '</tr><tr class="center"><td>quantidade:</td>';
			...
			$report_content .= '</tr><tr class="center"><td>Perdas:</td>';
			...
			if ($raport['c_units'][$pala_id] > 0) {
				...
				if ($raport['c_units'][$pala_id] == $raport['d_units'][$pala_id]) {
					if ($raport['to_user'] == $aktuuser) {
						$report_content .= 'Seu paladino morreu.';
						} else {
						$report_content .= 'O paladino é morto.';
						}
					} else {
					$report_content .= 'Paladino o imieniu '.$raport['def_pala_name'].' ,';
					if (!empty($raport['def_pala_item'])) {
						$report_content .= 'equipado com '.$pala_bonus[$raport['def_pala_item']][2];
						} else {
						$report_content .= 'sem o item.';
						}
					}
			...
			$report_content .= '<p>Todas as suas tropas cairam, nenhuma outra informação sobre o exército do oponente foi obtida</p>';
			}

		$report_content .= '</td></tr></table><br />';

		if ($def_out_units_see || count($raport['budynki']) > 1 || $def_out_res_see) {
			$report_content .= '<h4>Espionagem</h4>';
	
			$report_content .= '<table id="attack_spy" style="border: 1px solid rgb(222, 211, 185); padding: 0px 0px 0px 0px;">';
			if ($def_out_res_see) {
				$report_content .= '<tr><th>Recursos espionada:</th><td>';
				
					if ($raport['sorowce_poz']['0'] > 0) $report_content .= '<img src="/ds_graphic/wood.png" title="Madeira" alt="" />'.$raport['sorowce_poz']['0'].' ';
					if ($raport['sorowce_poz']['1'] > 0) $report_content .= '<img src="/ds_graphic/stone.png" title="Argila" alt="" />'.$raport['sorowce_poz']['1'].' ';
					if ($raport['sorowce_poz']['2'] > 0) $report_content .= '<img src="/ds_graphic/iron.png" title="Ferro" alt="" />'.$raport['sorowce_poz']['2'].' ';
				$report_content .= '</td></tr>';
				}
				
			if (count($raport['budynki']) > 1) {
				$report_content .= '<tr><th>Edifícios:</th><td>';
				
				$i = 0;
				
				foreach ($this->objects['cl_builds']->get_array("dbname") as $name => $dbname) {
					if ($raport['budynki'][$i] > 0) $report_content .= $this->objects['cl_builds']->get_name($dbname) . ' <b>(Stopie� '.$raport['budynki'][$i].')</b><br>';
					$i++;
					}
		...
		if (count($raport['e_units']) > 1) {
			$report_content .= '<h4>Tropas que estavam fora da aldeia</h4><table><tr>';
			foreach ($this->objects['cl_units']->get_array("dbname") as $name => $dbname) $report_content .= '<th width="35"><img src="/ds_graphic/unit/'.$dbname.'.png" title="'.$name.'" alt="" /></td>';
			$report_content .= '</tr><tr>';
		...
		if ($raport['hives']['0'] > 0 || $raport['hives']['1'] > 0 || $raport['hives']['2'] > 0) {
			...
			if ($raport['hives']['0'] > 0) {
				$report_content .= '<img src="/ds_graphic/wood.png" title="Madeira" alt="" />'.$raport['hives']['0'].' ';
				}
			if ($raport['hives']['1'] > 0) {
				$report_content .= '<img src="/ds_graphic/stone.png" title="Argila" alt="" />'.$raport['hives']['1'].' ';
				}
			if ($raport['hives']['2'] > 0) {
				$report_content .= '<img src="/ds_graphic/iron.png" title="Ferro" alt="" />'.$raport['hives']['2'].' ';
				}
			$report_content .= '</td><td>'.$raport['hives']['3'].'/'.$raport['hives']['4'].'</td></tr>';
			}
		
		if ($raport['to_user'] == $aktuuser && $def_out_units_see) {
			$report_content .= '<tr><th>Uwaga:</th><td>Suas tropas foram detectadas pelo inimigo.</td></tr>';
			}
		if (!empty($raport['pala_find_item'])) {
			$report_content .= '<tr><th>Ryczerz:</th><td>Seu Paladino encontra um objeto - '.$pala_bonus[$raport['pala_find_item']][2].'.</td></tr>';
			}
		if ($raport['ram'][0] != $raport['ram'][1]) {
			$report_content .= '<tr><th>Danos causados ​​por aríetes:</th><td colspan="2">Muralha destruída do nível<b>'.$raport['ram'][0].'</b> para nivel <b>'.$raport['ram'][1].'</b></td></tr>';
			}
		if ($raport['agreement'][0] != $raport['agreement'][1]) {
			$report_content .= '<tr><th>Moral:</th><td colspan="2">caiu de <b>'.$raport['agreement'][0].'</b> para <b>'.$raport['agreement'][1].'</b></td></tr>';
			}
		if ($raport['catapult'][0] != $raport['catapult'][1]) {
			$report_content .= '<tr><th>Danos causados ​​por catapultas:</th><td colspan="2">'.$this->objects['cl_builds']->get_name($raport['catapult'][2]).' destruído do nível <b>'.$raport['catapult'][0].'</b> para nivel <b>'.$raport['catapult'][1].'</b></td></tr>';
			}
