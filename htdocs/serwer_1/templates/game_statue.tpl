...
{if $village.$dbname > 0}
	...
	<div style="width: 840px; float: left;">
		...
		<map id="inv" name="inv">
			{foreach from=$user_pala_arr item=pala_item}
				<img style="position: absolute;" class="inv_{$pala_item}" src="graphic/inventory/{$pala_item}.png" title="{$pala_bonuses.$pala_item.2}"/>
	...
	<table class="vis">
		<tbody>
			...
			{foreach from=$units item=unit key=name}
				<tr>
					<td>
						<a href="javascript:popup('popup_unit.php?unit={$unit}', 520, 520)"> <img src="graphic/unit/{$unit}.png" alt="" /> {$name}</a>
					</td>
					<td>
