...
<tr><th>Origine</th>{foreach from=$cl_units->get_array("dbname") item=dbname key=name}<th width="40"><img src="graphic/unit/{$dbname}.png" title="{$name}" alt="" /></th>{/foreach}</tr>
	...
	<th>Chema&#355;i &#238;napoi</th></tr>
		...
		{foreach from=$outside_village_units key=id item=arr}
			<tr>
				...
				<td>
					<a href="game.php?village={$village.id}&amp;screen=place&amp;mode=units&amp;try=back&amp;unit_id={$id}">c&#226;&#355;iva</a> -
					<a href="game.php?village={$village.id}&amp;screen=place&amp;mode=units&amp;action=all_back&amp;unit_id={$id}&amp;h={$hkey}">toate</a>
				</td>
			</tr>
		{/foreach}
