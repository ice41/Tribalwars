{php}
	...
	if($from <> ""){
		...
		header("location: game.php?village=$id_sat&screen=ranking&mode=kill_player&type=att&site=$site&start=$from");
	}
	...
	if($name <> ""){
		...
		if(strlen($name) >= 3){
			...
			while($array2 = mysql_fetch_array($exec_sql2)){
				...
				$killed_units_att_user_search[] = $array2['killed_units_att'];
				$killed_units_att_rank_user_search[] = $array2['killed_units_att_rank'];
			}
		}else{
...
{/php}
	<tr>
		<td>{php}echo $killed_units_att_rank_user_search[$key];{/php}</td>
		...
		<td>{php}echo $killed_units_att_user_search[$key];{/php}</td>
	</tr>
{php}
			}
		}
...
<table class="vis" width="100%" style="border-spacing: 1px; background-color: #FEE47D; border:1px solid #fee47d;-moz-border-radius:5px 5px 5px 5px;-webkit-border-top-left-radius:5px;-webkit-border-top-right-radius:5px;-webkit-border-bottom-left-radius:5px;-webkit-border-bottom-right-radius:5px;">
	<tr width="100%">
	{if $site!=1}
		<td align="center"><a href="game.php?village={$village.id}&amp;screen=ranking&amp;mode=kill_player&amp;type=att&amp;site={$site-1}">&lt;&lt;&lt; anterior</a></td>
	{/if}
	<td align="center"><a href="game.php?village={$village.id}&amp;screen=ranking&amp;mode=kill_player&amp;type=att&amp;site={$site+1}">pr&oacute;xima &gt;&gt;&gt;</a></td>
	</tr>
</table><br />
<table class="vis" width="100%" style="border-spacing: 1px; background-color: #FEE47D; border:1px solid #fee47d;-moz-border-radius:5px 5px 5px 5px;-webkit-border-top-left-radius:5px;-webkit-border-top-right-radius:5px;-webkit-border-bottom-left-radius:5px;-webkit-border-bottom-right-radius:5px;">
	<tr>
		<td style="padding-right:10px; text-align: center;" width="45%">
			<form action="game.php?village={$village.id}&amp;screen=ranking&amp;mode=kill_player&type=att" method="post">
 		...
 		<td style="padding-right:10px;text-align: center;"  width="55%">
			<form action="game.php?village={$village.id}&amp;screen=ranking&amp;mode=kill_player&type=att&amp;search=" method="post">
