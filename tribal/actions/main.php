		...
		if (in_array($_POST['id'],$cl_builds->get_array("dbname"))) {
			if (in_array($_POST['id'],$fulfilled_builds_one)) {
				...
				if ($_NXT_BUILD_LEV > $cl_builds->get_maxstage($_POST['id'])) {
					$error = 'O nível máximo de expansão foi atingido a.';
					} else {
					...
					if ($village['r_wood'] >= $koszty_normal['wood'] && $village['r_stone'] >= $koszty_normal['stone'] && $village['r_iron'] >= $koszty_normal['iron']) {
						if (($maxfarm - $village['r_bh']) >= $koszty_normal['bh']) {
							if ($village['r_wood'] >= $koszty_all['wood'] && $village['r_stone'] >= $koszty_all['stone'] && $village['r_iron'] >= $koszty_all['iron']) {
								...
								$error = 'Muito pouco das matérias-primas para adicionar à estrutura de fila.';
								}
							} else {
							$error = 'Tem muito pouco espaço na Fazenda.';
							}
						} else {
						$error = 'tem muito poucas matérias-primas';
						}
					}
				} else {
				$error = 'Não atende Niono de requisitos para este edifício.';
				}
			} else {
			$error = 'Nie ma takiego budynku.';
			}
		}
		...
		if ($_GET['h'] == $session['hkey']) {
			...
			if (strlen($parsed_name) > 2) {
				if (strlen($parsed_name) > 40) {
					$error = 'Nazwa wioski mo¿e sk³adaæ siê z maksymalnie 40 znaków.';
					} else {
					if (substr_count($parsed_name," ") == strlen($parsed_name)) {
						$error = 'Nazwa wioski nie mo¿e sk³adaæ siê z samych spacji.';
						} else {
				...
				$error = 'Nazwa wioski musi ska³adaæ siê co najmniej z 3 znaków.';
				}
			} else {
			$error = 'B³¹d wykonywania akcji.';
			}
		...
		if ($_GET['h'] == $session['hkey']) {
			if (in_array($_POST['id'],$cl_builds->get_array("dbname"))) {
				if (in_array($_POST['id'],$fulfilled_builds)) {
					...
					$error = 'Nie spe³niono wymagañ do tego budynku.';
					}
				} else {
				$error = 'Nie ma takiego budynku.';
				}
