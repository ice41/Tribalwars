...
if ($config->get_cfg('admin_key') == 'actions_massiv_keyaaassd') {
	if ($_GET['akcja'] == 'add_new_server' && isset($_POST)) {
		...
		$contents_cfg_file = file_get_contents('serwer_'.$new_serwer.'/lib/config.php');
		...
		$cfg_file_server = fopen('serwer_'.$new_serwer.'/lib/config.php','w');
		fputs($cfg_file_server,$contents_cfg_file);
		fclose($cfg_file_server);
			
		header('LOCATION: admin.php?screen=create_new_server&suk=Adicionado um novo servidor ('.$new_serwer.')');
		exit;
		}
