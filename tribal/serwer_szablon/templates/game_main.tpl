		...
		<table class="vis">
			...
			{foreach from=$do_build item=item key=id}
				...
				{/if}
					...
					<td>
						<a href="javascript:ask('Czy na pewno chcesz przerwaæ budowanie?', 'game.php?village={$village.id}&amp;screen=main&amp;action=cancel&amp;id={$do_build.$id.r_id}&amp;mode=build&amp;h={$hkey}')">Przerwij</a>
					</td>
				</tr>
			{/foreach}
				...
				<tr>
					...
					{else}
						...
						{if $can_build.$dbname == 'not_enough_ress'}
							<td class="inactive"><span>Surowce dostêpne za <span class="timer_replace">{$res_timer.$dbname}</span></span><span style="display:none">Wystarczaj¹co surowców.</span></td>
		...
		<table class="vis">
			...
			{foreach from=$do_build item=item key=id}
				...
				{/if}
					...
					<td>
						<a href="javascript:ask('Czy na pewno chcesz przerwaæ budowanie?', 'game.php?village={$village.id}&amp;screen=main&mode=destroy&amp;action=cancel&amp;id={$do_build.$id.r_id}&amp;h={$hkey}')">Przerwij</a>
					</td>
				</tr>
			{/foreach}
		</table>
		...
		<table class="vis">
			...
			{foreach from=$do_destory item=item key=id}
				...
				{/if}
					...
					<td>
						<a href="javascript:ask('Czy na pewno chcesz przerwaæ wyburzanie?', 'game.php?village={$village.id}&amp;screen=main&mode=destroy&amp;action=cancel_dest&amp;id={$do_destory.$id.r_id}&amp;h={$hkey}')">Przerwij</a>
					</td>
				</tr>
			{/foreach}
		</table>
