...
{if $show_build}
	...
	{if $mode == 'poj_monety'}
		...
		<form action="game.php?village={$village.id}&amp;screen={$dbname}&amp;action=train&amp;h={$hkey}" method="post" onsubmit="this.submit.disabled=true;">
			<table class="vis">
				...
				{foreach from=$units key=unit_dbname item=name}
					<tr>
						<td><a href="javascript:popup('popup_unit.php?unit={$unit_dbname}', 520, 520)"> <img src="/ds_graphic/unit/{$unit_dbname}.png" alt="" /> {$name}</a></td>
		...
		{if $ag_style==0}
			...
			<table class="vis">
				<tr><td>Poziom szlachty:</td><td>{$snobs_stage|format_number}</td></tr>
				<tr><td>- Szlachcice dostêpni:</td><td>{$snobs_dostepne|format_number}</td></tr>
				<tr><td>- Szlachcice w produkcji:</td><td>{$snobs_produkcja|format_number}</td></tr>
				<tr><td>- Szlachcice osadzenie w wioskach:</td><td>{$snobs_in_vgs|format_number}</td></tr>
				<tr><th>Mo¿na wyprodukowaæ:</th><th>{$snobs_canpr|format_number}</th></tr>
			</table>
		{elseif $ag_style==1}
			<h4>Liczba szlachciców, których mo¿na wytrenowaæ</h4>
			<table class="vis">
				...
				{foreach from=$masowe_wybijanie item=wioska}
					<tr{if $wioska.id == $village.id} class="selected"{else}{if !$wioska.parzysta_liczba} class="row_b"{else} class="row_a"{/if}{/if}>
						...
						<span style="font-weight:bold; color: #804000; cursor: pointer;" onclick="insertNumId('{$wioska.id}', '{$wioska.max_monets_can_coin}');return false;">
							(maks. {$wioska.max_monets_can_coin} <img alt="Moneta" src="/ds_graphic/gold.png" style="position: relative;top: 3px;"/>)
						</span>
						</td>
					</tr>
				{/foreach}
