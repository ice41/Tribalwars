	...
	if ($_GET['h'] == $session['hkey']) {
		...
		if ($counts > 0) {
			...
			if ($required_offer_dealers > $inside_dealers) {
				$error = "Posiadasz za ma³o kupców, aby przyj¹æ ofertê";
				} else {
				if ($req_res > $village[$vresname]) {
					$error = "Posiadasz za ma³o surowców, aby przyj¹æ ofertê";
					}
				}
			...
			if (empty($error)) {
				...
				//Dodaj raport:
				$title = $user["username"] . " przyj¹ twoja ofertê";
