...
if (is_array($user_sid)) {
	$user = sql("SELECT id,villages,aktu_vpage,groups,aktu_group,villages_per_page FROM `users` WHERE `id` = '".$user_sid['userid']."'","assoc");
...
<center>
	<div class="vis_item" align="center">
		<? if (is_array($groups_array_as_all)): ?>
			<? foreach ($groups_array_as_all as $group): ?>
				<? if ($group == $user['aktu_group']): ?>
					<strong class="group_tooltip">&gt;<? if ($group === 'all'): echo 'wszystkie'; else: echo $group; endif;?>&lt; </strong>
				<? else: ?>
					<a class="group_tooltip" href="#" onclick="return inlinePopup(event, 'bookmark', 'villages.php?action=change_group&group=<? echo base64_encode($group);?>&h=<? echo $user_sid['hkey'];?>', popup_options)">[<? if ($group === 'all'): echo 'wszystkie'; else: echo $group; endif;?>]</a>
				<? endif; ?>	
			<? endforeach; ?>
		<? endif; ?>	
	</div>
	...
	<table class="vis padding2" width="100%">
		<tr>
			<th height="20px">Twoje wioski:</th>
		</tr>
