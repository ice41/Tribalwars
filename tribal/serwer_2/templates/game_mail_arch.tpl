{if empty($view)}
	...
	{/if}
	<tr><th>Assunto</th><th width="160">Sem Filtro</th><th width="160">Remetente</th><th width="140">Data</th></tr>
		...
		{/foreach}
		{if count($mails)>0}
			<tr><th><input name="all" type="checkbox" onclick="selectAll(this.form, this.checked)">Marcar tudo</th><th colspan="3"></th></tr>
		{/if}
	</table>
...
{else}
	{if empty($error)}
		...
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=arch">Arquivar</a></td>
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;action=del&amp;id={$mail.id}&amp;mode=arch&amp;h={$hkey}">Exportar</a></td>
		</tr>
		</table>
		
		<table class="vis" width="100%">
		<tr><th width="140">Remetente</th><th width="300">{$mail.subject}</th></tr>
		<tr><td>Remetente</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.from_id}">{$mail.from_username}</a></td></tr>
		<tr><td>Distinataria</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.to_id}">{$mail.to_username}</a></td></tr>
