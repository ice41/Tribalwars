...
<tr><th>Jednostki</th>{foreach from=$cl_units->get_array("dbname") item=dbname key=name}<th width="40"><img src="/ds_graphic/unit/{$dbname}.png" title="{$name}" alt="" /></th>{/foreach}</tr>
	...
	<th>Rozkaz</th></tr>
		...
		{foreach from=$outside_village_units key=id item=arr}
			<tr>
				...
				<td>
					<a href="game.php?village={$village.id}&amp;screen=place&amp;mode=units&amp;try=back&amp;unit_id={$id}">Czêœæ</a> /
