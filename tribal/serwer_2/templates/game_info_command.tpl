{if $command_exists}
	...
	{if $type=='own'}
		...
		<tr><td>Jogador:</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mov.to_userid}">{$mov.to_username}</a></td></tr>
		<tr><td>Duração:</td><td>{$mov.duration|format_time}</td></tr>
		<tr><td>Chegada:</td><td>{$mov.arrival|format_date}</td></tr>
		<tr><td>Regresso em:</td><td><span class="timer">{$mov.arrival_in|format_time}</span></td></tr>
		...
		<tr><td colspan="2"><a href="game.php?village={$village.id}&amp;&amp;screen=place">&raquo; Praça de Reuniões</a></td></tr>
		{if $mov.cancel}
			<tr><td colspan="2"><a href="game.php?village={$village.id}&screen=place&action=cancel&id={$mov.id}&h={$hkey}">&raquo; Cancelar</a></td></tr>
		{/if}	
		...
		{if $mov.wood!=0 || $mov.stone!=0 || $mov.iron!=0}
			<table class="vis"><tr>
			<td>£upy:</td><td>{if $mov.wood>0}<img src="/ds_graphic/holz.png" title="Madeira" alt="" />{$mov.wood|format_number} {/if}{if $mov.stone>0}<img src="/ds_graphic/lehm.png" title="Argila" alt="" />{$mov.stone|format_number} {/if}{if $mov.iron>0}<img src="/ds_graphic/eisen.png" title="Ferro" alt="" />{$mov.iron|format_number} {/if}</td>
			</tr></table>
		{/if}
	{else}
		...
		<tr><td>Chegada:</td><td>{$mov.arrival|format_date}</td></tr>
		<tr><td>Retorno:</td><td><span class="timer">{$mov.arrival_in|format_time}</span></td></tr>
		</table>	
	{/if}
{else}
