...
class forum {
	...
	function forum($uinfo) {
		...
		if (!empty($this->user_info['ally']) && $this->user_info['ally'] != '-1') {
			...
			if ($ogolne_forum_plemie == 0) {
				$this->add_forum(parse("Ogólnie"),0);
				mysql_query("UPDATE `ally` SET `add_ffid` = '1' WHERE `id` = '".$this->user_info['ally']."'");
				}
			} else {
			die ("B³¹d wczytywania pliku - sprawdŸ czy parametr funkcji forum 'uinfo' jest tablic¹.");
			}
		}
	...
	function get_pool_form($tid,$vid,$link) {
		...
		//SprawdŸ czy gracz g³osowa³, czy nie:
		...
		foreach ($odp_array as $id => $odpowiedz) {
			...
			if ($finfo['show_wyn'] || $is_user_voted) {
				...
				if ($wyn_array[$id] != 0) {
					...
					$return .= '<td>'.$wyn_array[$id].' g³osów ('.$proc.'%)</td>';
					} else {
					$return .= '<td></td>';
					$return .= '<td>(0%)</td>';
					}
		...
		if (!$is_user_voted) {
			$return .= '<tr><td>';
			$return .= '<input type="submit" name="sub" value="G³osowaæ"/>';
			$return .= '</td></tr>';
			}
