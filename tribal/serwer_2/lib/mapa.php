...
class mapa {
	...
	function status_osady($impl_coord,$auser,$aally) {
		...
		if ($atts_to_user > 0) {
			$return = '<img style="position: relative; top: -36px; left: 35px;" src="/ds_graphic/map/att_to.PNG"/>';
			} else {
			$in_reservations = sql("SELECT COUNT(id) FROM `rezerwacje` WHERE `do_wioski` = '".$this->villages[$impl_coord]['vid']."' AND `od_plemienia` = '$aally'",'array');
			if ($in_reservations > 0) {
				$return = '<img style="position: relative; top: -36px; left: 35px;" src="/ds_graphic/map/reserved_player.png"/>';
				} else {
				...
				if ($atts_from_user > 0) {
					$return = '<img style="position: relative; top: -36px; left: 35px;" src="/ds_graphic/map/attack.png"/>';
					} else {
					...
					if ($returns_from_user > 0) {
						$return = '<img style="position: relative; top: -36px; left: 35px;" src="/ds_graphic/map/return.png"/>';
						}
					}
				}
			}
	...
	function get_player_info($i) {
		...
		$return .= "'";
		if (is_array($u)) {
			$return .= entparse($u['username']) .' ('.$this->html_num($u['points']).' punktów)';
		    }
		$return .= "'";
		return $return;
	    }
	...
	function get_ally_info($i) {
		...
		$return .= "'";
		if (is_array($a)) {
			$return .= entparse($a['name']) .' ('.$this->html_num($a['points']).' punktów)';
		    }
		$return .= "'";
		return $return;
	    }
    ...
    function rysuj_minimape($xxx,$yyy,$rozmiar_x,$rozmiar_y,$rozmiar_px,$aktuosada,$aktugracz,$aktusojusz) {
		...
		imagepng($im, 'graphic/'.$aktugracz.'.png', 100);
		imagedestroy($im);
		}
    }
?>