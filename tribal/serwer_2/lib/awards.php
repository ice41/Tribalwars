...
class awards {
	var $awards_configs = array (
		...
		'odznaczenie_podbicia' => array(
			...
			4,
			'Podbój',
			'Iloœæ podbitych wiosek do tej pory wiosek: %s!',
	...
	function get_worlds_awards($user_tw,$aktuuser) {
		...
		if (!empty($tw_user_serwers)) {
			...
			if (is_array($array_servers) && count($array_servers) > 0) { 
				$return .= '<table class="vis" width="100%"/><tr><th colspan="2">Zdobyte odznaczenie na innych œwiatach</th></tr>';
				foreach ($array_servers as $serwer) {
					//Po³¹czyæ z baz¹ danych docelowego serwera:
					cdb_server($serwer);
					
					//Wybraæ id usera na podstawie tw_id:
					$user_server_id = sql("SELECT `id` FROM `users` WHERE `tw_id` = '$user_tw'",'array');
					
					//Pobraæ sk³ad odznaczeñ:
					$awards_contents = $this->get_user_awards_mini($user_server_id,$aktuuser,'profile');
					
					if (!empty($awards_contents)) {
						$return .= '<td>œwiat '.$serwer.'</td><td>'.$awards_contents.'</td>';
						}
					}
				$return .= '</table>';
				}
