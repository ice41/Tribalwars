...
for ($i = $village[$_GET['screen']]; $i <= $i_conq; $i++) {
	if ($i == $village[$_GET['screen']]) {
		$opis = 'A produção atual';
		} else {
		$opis = 'Produção a (nível '.$i.')';
		}
...
$res_dtp = sql("SELECT `allw` FROM `villages` WHERE `id` = '".$village["id"]."'","array");

if ($_GET["action"] === "resetCounter" && $_GET["h"] == $session['hkey']) {
	mysql_query("UPDATE `villages` SET `allw` = '0' WHERE `id` = '".$village["id"]."'");
