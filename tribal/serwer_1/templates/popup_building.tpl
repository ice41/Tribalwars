									...
									<table class="vis">
										...
										<tr>
											{foreach from=$cl_builds->get_needed_by_dbname($building) key=n_building item=n_stage}
												<td><a href="popup_building.php?building={$n_building}">{$cl_builds->get_name($n_building)}</a> (Poziom {$n_stage})</td>
											{/foreach}
										</tr>
									</table>
									<br>
								...
								<table class="vis">
									...
									{section name=foo start=1 loop=$cl_builds->get_maxstage($building)+1 step=1}
										<tr>
											...
											<td>
												<img src="/ds_graphic/holz.png" title="Drewno" alt="" />{$cl_builds->get_wood($building,$smarty.section.foo.index)} 
												<img src="/ds_graphic/lehm.png" title="Ceg³y" alt="" />{$cl_builds->get_stone($building,$smarty.section.foo.index)} 
												<img src="/ds_graphic/eisen.png" title="¯elazo" alt="" />{$cl_builds->get_iron($building,$smarty.section.foo.index)} 
											</td>
											<td>
