...
<table>
<tr><td>Distinatário:</td><td><input type="text" name="to" size="50" value="{$inputs.to}" />
{if $user.ally_mass_mail==1 && $user.ally!=-1}<a href="javascript:popup_scroll('igm_to.php?', 250, 300)">Correio em Massa</a>{/if}</td></tr>
<tr><td>Temat:</td><td><input type="text" name="subject" size="50" value="{$inputs.subject}" /></td></tr>
<tr><td colspan="2">
</td></tr>
<tr><td colspan="2"><textarea name="text" cols="70" rows="20">{$inputs.text}</textarea></td></tr>
<tr><td><input type="submit" name="preview" value="Podgl¹d" /> <input type="submit" name="send" value="Enviar"> </td>
