...
function getrandomxyforcircle($okrag,$kierunek) {
	...
	/*
	KIERUNKI:
		PW - Po≥udniowy wschÛd
		PZ - Po≥udniowy zachÛd
		OW - PÛ≥nocny wschÛd
		OZ - PÛ≥nocny zachÛd
		R - Losowy
	*/
...
function create_villages($gracz,$ilosc,$kierunek) {
	...
	if ($gracz == '-1') {
		...
		$nazwa = 'Wioska ' . sql("SELECT `username` FROM `users` WHERE `id` = '$gracz'",'array') . 's';
		}
...
function reload_all_rangs() {
	...
	//Reload rankingÛw - pokonanych jako obroÒca graczy:
	...
	//Reload rankingÛw - pokonanych jako napastnik graczy:
	...
	//Reload rankingÛw - pokonanych ≥πcznie graczy:
	...
	//Reload rankingÛw plemion - pokonanych jako obroÒca:
	...
	//Reload rankingÛw plemion - pokonanych jako napastnik:
	...
	//Reload rankingÛw plemion - pokonanych ≥πcznie:
...
function cdb_server($serwer = '') {
	if (!empty($serwer) && is_numeric($serwer)) {
		...
		$config_file = "../serwer_$serwer/lib/config.php";
		if (file_exists($config_file)) {
			require($config_file);
			$db->connect($config['db_host'],$config['db_user'],$config['db_pw'],$config['db_name']);
			} else {
			die ("Falalny b≥πd w funkcji cdb_server() - brak pliku konfiguracyjnego serwera <b>$serwer</b>");
			}
		} else {
		global $db,$config;
		$db->connect($config['db_host'],$config['db_user'],$config['db_pw'],$config['db_name']);
...
function get_first_village_group($uid) {
	...
	if (empty($group)) {
		die ("B≥πd prasowania - nie istnieje gracz o id='$uid'");
		} else {
		if ($group === 'all') {
...
function sim_battle($wojsko_obroncy,$wojsko_napastnika,$mor_level,$cel_katapolt_level,$szeczescie,$morale,$noc,$cel_budynek_nazwa = null,$att_pala_item,$def_pala_item) {
	...
	if ($katapulty > 0) {
		...
		/*Zabespieczenie przed doszczÍtnym przez katapulty zniszczeniem ratusza,spichlerza,kryjÛwki,zagrody i placu*/
...
//UsuÒ plemiÍ bez graczy
function delte_empty_ally() {

			//UsuÒ zaproszenia do sojuszu docelowego:
			mysql_query("DELETE FROM `ally_invites` WHERE `from_ally` = '".$user['ally']."'");
				
			//UsuÒ eventy z sojuszu docelowego:
