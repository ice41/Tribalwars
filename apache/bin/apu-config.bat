...
goto endofperl
@rem ';
#!D:\servers\servers\tralha tribalwars\Empire of War\perl\bin\perl.exe
#line 15
use strict;
use warnings;
use Getopt::Long;
...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache\include];
my ${exec_prefix} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache];
my ${datadir} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache\lib];
my ${APRUTIL_DOTTED_VERSION} = q[1.2.3];
my ${CFLAGS} = q[ /nologo /MD /W3 /O2 /D WIN32 /D _WINDOWS /D NDEBUG ];
my ${SHELL} = q[D:\servers\servers\tralha tribalwars\Empire of War\WINDOWS\system32\cmd.exe];
my ${CPPFLAGS} = q[];
my ${EXTRA_INCLUDES} = q[];
my ${APRUTIL_SOURCE_DIR} = q[];
my ${prefix} = q[D:\servers\servers\tralha tribalwars\Empire of War\apache];
